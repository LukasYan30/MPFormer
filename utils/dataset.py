...
def get_patch(img_in, img_tar, patch_size, scale=1, ix=-1, iy=-1):
    iw, ih = img_in.size   # PIL: (width, height)

    tp = patch_size * scale
    ip = patch_size

    if iw < ip or ih < ip:
        raise ValueError(f"Patch size {ip} is larger than image size {(iw, ih)}")

    if ix == -1:
        ix = random.randrange(0, iw - ip + 1)  # x
    if iy == -1:
        iy = random.randrange(0, ih - ip + 1)  # y

    tx, ty = scale * ix, scale * iy

    # crop(left, upper, right, lower) = (x1, y1, x2, y2)
...
class DatasetFromFolder(data.Dataset):
    def __init__(
        ...
        label_filenames = [
            join(data_root, "reference", x)
            for x in listdir(join(data_root, "reference"))
            if is_image_file(x)
        ]
